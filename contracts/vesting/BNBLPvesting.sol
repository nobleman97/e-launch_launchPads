...
contract BNBLaunchPad is Initializable, vestinginfo {
    ...
    * */
    ...
    function amountsNeededForLiquidity() public view returns(
        ...
        * ...
        ...
        if(BNBFee_ == 5){
            ...
            uint totalTokensForLiquidity_wei = ((totalBNBReceivedInAllTier_notWei * 95) * (_listingRate * _liquidityPercent)) * 1e2;
            ...
            uint totalTokensForLiquidity_wei = ((totalBNBReceivedInAllTier_notWei * 98) * (_listingRate * _liquidityPercent)) * 1e2;
    ...
    function finalize() public onlyOwner{
        ...
        _finalizeTime = block.timestamp;
    }

    function calculateReleasedTokens() public view returns(uint _amountClaimable) {

        require(presaleFinalized == true, "claimTokens: User cannot calculate tokens till sale is finalized");

        uint amountClaimable;

        for (uint _pocketNumber = 0; _pocketNumber < numberOfCliffs; _pocketNumber++){
            uint currentTime = block.timestamp;
            uint unlockTime = _finalizeTime + (cliff * _pocketNumber);     

            if (hasClaimedFromPocket[msg.sender][_pocketNumber] == true  ){
                continue;
            }
            ...
            amountClaimable += (claimableTokenBalance[msg.sender] * percentPerCliff[_pocketNumber]) / 100 ;
    ...
    function claimFromPocket() internal{
        ...
        for (uint _pocketNumber = 0; _pocketNumber < numberOfCliffs; _pocketNumber++){
            ...
            amountClaimable += (claimableTokenBalance[msg.sender] * percentPerCliff[_pocketNumber]) / 100 ;            
            hasClaimedFromPocket[msg.sender][_pocketNumber] = true;
        }

        require(amountClaimable > 1 * 1e16, "You must claim at least 0.01 token");

        ERC20Interface.safeTransfer(msg.sender, amountClaimable);
    }

    function claimTokens() external {
        if (vestingIsActivated == false) {
        
            // Add boolean to ensure that liquidity pool has been created
            require(presaleFinalized == true, "claimTokens: User cannot claim tokens till sale is finalized");
        ...
        } else if (vestingIsActivated == true)  {
            require(presaleFinalized == true, "claimTokens: User cannot claim tokens till sale is finalized");
            claimFromPocket();
        }
    }
