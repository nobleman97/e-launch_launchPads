    ...
    function totalTokensNeeded(
        ...
        * return tokensNeeded, _BNBFee
        ...
        if(_BUSDFee == 4){
            ...
            uint totalTokensForLiquidity_wei = ((hardCap * 96) * (listingRate * liquidityPercent)) / 10**4;
           ...
           uint totalTokenBeingSold_wei = (hardCap * presaleRate) * 10**18;
            uint totalTokensForLiquidity_wei = ((hardCap * 98) * (listingRate * liquidityPercent)) / 10**4;

            uint totalTokensNeeded_wei = totalTokenBeingSold_wei + totalTokensForLiquidity_wei;

            uint tokensFeeToCharge = (totalTokenBeingSold_wei * 15) / 1000;

            totalTokensNeeded_wei = ((1015 * (hardCap * presaleRate)) / 10**3) + totalTokensForLiquidity_wei;
